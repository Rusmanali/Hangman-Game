...
try:
    ...
    messagebox.showerror("Error", f"Could not load sound files: {e}")
    raise
...
def game_window(category):
    ...
    for let in 'abcdefghijklmnopqrstuvwxyz':
        try:
            letter_images[let] = PhotoImage(file=resource_path(f"{let}.png"))
        except Exception as e:
            messagebox.showerror("Error", f"Could not load letter image: {e}")
            letter_images[let] = PhotoImage()

    hangman_images = []
    for i in range(1, 8):
        try:
            hangman_images.append(PhotoImage(file=resource_path(f"h{i}.png")))
        except Exception as e:
            messagebox.showerror("Error", f"Could not load hangman image: {e}")
    ...
    hearts = []
    try:
        heart_full = ImageTk.PhotoImage(Image.open(resource_path("heart1.png")).resize((20, 20), Image.LANCZOS))
    except Exception as e:
        messagebox.showerror("Error", f"Could not load heart image: {e}")
        heart_full = PhotoImage()
    try:
        heart_empty = ImageTk.PhotoImage(Image.open(resource_path("heart2.png")).resize((20, 20), Image.LANCZOS))
    except Exception as e:
        messagebox.showerror("Error", f"Could not load heart image: {e}")
    ...
    try:
        ex_img = PhotoImage(file=resource_path('exit.png'))
    except Exception as e:
        messagebox.showerror("Error", f"Could not load exit image: {e}")
        ex_img = PhotoImage()
